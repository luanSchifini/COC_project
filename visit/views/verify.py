...
from visit.models import Guest, Student
from visit.services import get_guest, increment_student_visits
...
def _render_verify(request, guest_id, common_context=None):
    return render(request, 'verify.html', {"user_id": guest_id})
...
def _verify_student_visit(request, guest_instance, guest_id, student_instance, student_id, common_context=None):
    if not _is_visit_valid(student_instance, guest_instance):
        ...
        return _render_verify(request, guest_id, common_context)
